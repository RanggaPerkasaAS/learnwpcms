 ...
 * @return array
 */
function tastypress_body_classes( $classes ) {
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'tastypress-selected-sidebar', true );
	...
	if(is_front_page() && ($options['home_layout'] != 'fourth-design')){
		$classes[] = 'featured-slider-enabled';
	}

	if(has_header_image() && ($options['home_layout'] == 'default-design')){
		$classes[]='header-image-enabled';
	}else{
		$classes[]= 'header-image-disabled';
	}
...
add_filter( 'body_class', 'tastypress_body_classes' );
